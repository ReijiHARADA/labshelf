...
/** 縦横比から高さ・幅を決める（aspect ratio は維持） */
export function getBookVisualSize(aspectRatio: number): { width: number; height: number } {
  ...
  let height = 340;
  if (safeRatio > 0.9) height = 300;
  else if (safeRatio < 0.52) height = 370;
...
/**
 * 表紙・スパイン・背面の3面を持つ CSS 3D 本オブジェクト。
 *
 * 3D ボックスモデル（中心 = 表紙の中心）:
 *   front: translateZ(+S/2)
 *   back:  rotateY(180deg) translateZ(+S/2)
 *   left:  left: -(S/2), transform: rotateY(-90deg)   ← スパイン
 *   right: left: W-(S/2), transform: rotateY(90deg)   ← 小口（スパイン色）
 *
 * perspective は親ステージ側で1回だけ設定する。
 * このコンポーネント内には perspective を持たない。
 */
export function Book3DCard({
  ...
  const titleText = book.title.length > 20 ? book.title.slice(0, 19) + '…' : book.title;
  const authorText = book.author.length > 14 ? book.author.slice(0, 13) + '…' : book.author;
  ...
  return (
    // Outer wrapper: scale + opacity (non-3D transform to keep 3D context clean)
    <div
      role="button"
      tabIndex={0}
    ...
    >
      ...
      >
        ...
        >
          ...
          <div
            style={{
              ...
              inset: 0,
              background:
                'linear-gradient(135deg, rgba(0,0,0,0.25) 0%, rgba(0,0,0,0.08) 60%, rgba(255,255,255,0.04) 100%)',
              pointerEvents: 'none',
            }}
          />
        </div>
        ...
        {/* ── 左面（スパイン / 背表紙）── */}
        {/* left: -(S/2) で中心を x=0 (表紙の左端) に揃え、rotateY(-90deg) で折り曲げ */}
        <div
          style={{
            position: 'absolute',
            top: 0,
        ...
        {/* ── 右面（小口）── スパイン色で対称に */}
        {/* left: W-(S/2) で中心を x=W (表紙の右端) に揃え、rotateY(90deg) で折り曲げ */}
        <div
          style={{
            position: 'absolute',
            top: 0,
      ...
      {isActive && (
        <div
          ...
          style={{
            ...
            inset: -3,
            borderRadius: 2,
            boxShadow: '0 0 0 2px rgba(99,102,241,0.85)',
            pointerEvents: 'none',
          }}
        />
      )}
...
function FallbackFace({
  ...
  return (
    ...
    >
      <BookOpen style={{ width: 36, height: 36, color: 'rgba(255,255,255,0.4)' }} />
      {!dimmed && title && (
        <p
          style={{
